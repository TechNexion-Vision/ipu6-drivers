...
struct tevs {
	...
	struct tevs_platform_data *platform_data;
};
...
static int tevs_pca953x_i2c_wr8(struct tevs *tevs, u8 reg, u8 val)
{
	// struct device *dev = tevs->dev;
	struct i2c_client *client = to_i2c_client(tevs->dev);
	int ret;
	struct i2c_msg msg;
	u8 pca953x_addr = 0x27;
	u8 data[2];

	msg.addr = pca953x_addr;
	msg.buf = data;
	msg.len = 2;
	msg.flags = 0;
...
static int tevs_rpi22_trick_init(struct tevs *tevs)
{
	struct device *dev = tevs->dev;
	int ret = 0;

	ret += tevs_pca953x_i2c_wr8(tevs, 0x01, 0xbf);
	ret += tevs_pca953x_i2c_wr8(tevs, 0x02, 0x00);
	ret += tevs_pca953x_i2c_wr8(tevs, 0x03, 0xbf);

	if (ret < 3) {
		dev_err(dev, "%s(): pca953 initial fail\n", __func__);
		return -EINVAL;
	}
...
static int tevs_pca953x_standby(struct tevs *tevs, u8 standby)
{
	struct device *dev = tevs->dev;
	u8 val = 0xff;
	int ret = 0;

	// Check the value to set bit 6
    if (standby == 0) {
		val &= ~(1 << 6); // Clear bit 6 to 0
    } else {
        val |= (1 << 6); // Set bit 6 to 1
    }

	ret = tevs_pca953x_i2c_wr8(tevs, 0x01, val);
	if (ret < 1) {
		dev_err(dev, "%s(): set standby pin fail\n", __func__);
		return -ret;
	}
...
static int tevs_power_on(struct tevs *tevs)
{
	...
	gpio_set_value(tevs->platform_data->reset_pin, 1);
...
static int tevs_set_ae_mode(struct tevs *tevs, s32 mode)
{
	...
	tevs->exp_time_ctrl->cur.val = be32_to_cpup((__be32 *)exp) &
...
static int tevs_set_bsl_mode(struct tevs *tevs, s32 mode)
{
	...
	case TEVS_BSL_MODE_FLASH_IDX:
		gpio_set_value(tevs->platform_data->reset_pin, 0);
		usleep_range(9000, 10000);
		tevs_pca953x_standby(tevs, 1);
		msleep(100);
		gpio_set_value(tevs->platform_data->reset_pin, 1);
		usleep_range(9000, 10000);
		tevs_pca953x_standby(tevs, 0);
		msleep(100);
	...
	for (i = 0; i < ARRAY_SIZE(tevs_ctrls); i++) {
		...
		case V4L2_CID_EXPOSURE_AUTO:
			...
			case TEVS_AE_CTRL_AUTO_GAIN: {
				u8 exp[4] = { 0 };
				ret += tevs_i2c_read(
					tevs, TEVS_AE_MANUAL_EXP_TIME, exp, 4);
				tevs->exp_time_ctrl->cur.val =
					be32_to_cpup((__be32 *)exp) &
					TEVS_AE_MANUAL_EXP_TIME_MASK;
				ctrl->default_value = ctrl->cur.val =
...
static int tevs_probe(struct i2c_client *client)
{
	...
	if (tevs == NULL) {
		dev_err(dev, "tevs allocate memory failed\n");
		return -ENOMEM;
	}

	tevs->platform_data = client->dev.platform_data;
	if (tevs->platform_data == NULL) {
		dev_err(&client->dev, "no platform data provided\n");
		return -EINVAL;
	}

	tevs->dev = &client->dev;
	ret = tevs_rpi22_trick_init(tevs);
	if (ret != 0) {
		dev_err(dev, "tevs rpi22 trick control failed\n");
		return -EINVAL;
	}
	...
	tevs->data_lanes = tevs->platform_data->lanes;
...
error_power_off:
	tevs_power_off(tevs);
	dev_err(dev, "%s Probe Failed", tevs->v4l2_subdev.name);
